...
| **ioda** | `IODA_BUILD_LANGUAGE_FORTRAN` (ON), `BUILD_PYTHON_BINDINGS` (ON if pybind11 found), `ENABLE_IODA_DOC` (OFF). Requires CMake ≥3.23 (`file(COPY_FILE)`/`file(CREATE_LINK)` replacing `execute_process`, PRs #1841/#1846). |
...
| **saber** | vader, optional FFTW, ECTRANS, gsibec 1.4.4, PyTorch/LibTorch (for TorchBalance) |
| **vader** | optional GSW (enables ocean recipes) |
| **fv3-jedi** | fv3-jedi-lm ≥1.5.0, FMS 2023.04, optional crtm/gsibec 1.4.4/GEOS GCM/UFS |
...
| **fv3-jedi** | ~100+ YAML configs; test CMakeLists.txt ~2700 lines. `fv3jedi_hyb-4denvar_geos_sondes` and `fv3jedi_hyb-4denvar_llens_geos` carry no `LABELS`, so they run in the default tier and in CI (the former `tier2` / `ci_flake_disable` labels were dropped in fv3-jedi#1530). |
