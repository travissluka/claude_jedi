...
> Last updated against commit `159c068e` (2026-06-10). Run `cd bundle/oops && git log --oneline 159c068e..HEAD` to see what changed since.
>
...
| `util/` | Utilities: `DateTime`, `Duration`, `Logger`, `ConfigFunctions`, MPI helpers, Fortran interop. Includes `Factory.h` (generic factory template with variadic maker args) and `FunctionSpaceHelpers.h` (`getSizeOwned`, `countOwned`, `executeFunc` dispatcher over concrete ATLAS function-space types; the `isRegional` helper was removed in PR #3313 along with the `UnstructuredInterpolator` regional-check flag). |
